...
export const ModalComponent: FC<IModalContentProps> = (props) => {
  ...
  return (
    <>
      {typeModal === "question" ? (
        <Modal title="Добавить вопрос" open={isModalOpen} onOk={()=>onHandleOk('question')} onCancel={onHandleCancel} style={modalStyle}>
        ...
        <Modal title="Регистрация" open={isModalOpen} onOk={() => onHandleOk('registration')} onCancel={onHandleCancel} style={modalStyle}>
          ...
          <Select style={selectStyle} onChange={(value) => onHandleChangeSelect(value)} options={optionSkill} placeholder='Уровень подготовки' />
        </Modal>
      )}
    </>
  );
